...
import model.StdPlayer_AI_hard;
...
public class Othello {
	...
	private JMenuItem jmiChangeBGColor;
	...
	private Icon PionB = new ImageIcon("src/img/PionB.png");
	private Icon PionN = new ImageIcon("src/img/PionN.png");
	...
	private void createView() {
		...
		jmiChangeBGColor = new JMenuItem("Changez la couleur de fond");
		jmiKnowNextMove = new JMenuItem("Savoir les prochains coups a jouer");
		jmGameOption.add(jmiChangeBGColor);
	...
	private void createController() {
		...
		model.addPropertyChangeListener(PROP_FINISHED, 
				new PropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent evt) {
						...
						if(nbL + nbW == 64) {
							JOptionPane.showMessageDialog(mainFrame, "Le gagnant est " + winner.getName() 
																 + " sur un score de " + nbW
																 + " ра " + nbL, "Victoire !", JOptionPane.INFORMATION_MESSAGE);
						} else {
							JOptionPane.showMessageDialog(mainFrame, "Le gagnant est " + winner.getName() 
		...
		resetGame.addActionListener(new ActionListener( ) {
			...
			public void actionPerformed(ActionEvent e) {
				if(!model.isFinish()) {
					int rep = JOptionPane.showConfirmDialog(mainFrame,"La partie est encore en cours voulez vous quitter ?", "Quitter la partie",JOptionPane.YES_NO_OPTION);
		...
		levelThree.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				testTypeGameAlreadyChoose();
				model.setPlayer(new StdPlayer("Joueur 1", model));
				model.setPlayer(new StdPlayer_AI_hard("IA", model));	
				model.setTypeGame(1);
				refreshGoButton();
				wichPlay.setText("Veuillez Lancer la partie");
				refreshGoButton();
		...
		jmiChangeBGColor.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				Object[] tabCouleur = {Color.RED, Color.GREEN};
				Object col = JOptionPane.showInputDialog(
						mainFrame,
						"Choisissez une Couleur de fond",
						"Choix de Couleur",
						JOptionPane.QUESTION_MESSAGE, null,
						tabCouleur, tabCouleur[0]);
				changeBGColorCases((Color) col);
			}
		...
		jmInfo.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					java.awt.Desktop.getDesktop().open(new File("Notice_Utilisation.pdf"));
	...
	private void refreshGoButton() {
		if((model.getPlayers().size() == 1 && model.getTypeGame() == 1) || (model.getPlayers().size() == 2 && model.getTypeGame() == 2) 
	...
	public void changeBGColorCases(Color c) {
		for (int i = 0 ; i < 8 ; i++) {
			for (int j = 0 ; j < 8 ; j++) {
	...
	public void refreshCases() {
		...
		for (int i = 0 ; i < 8 ; i++) {
			for (int j = 0 ; j < 8 ; j++) {
				...
				if((model.isNextMoves(i, j) && !(model.getNextPlayer() instanceof StdPlayerAI_normal) && !(model.getNextPlayer() instanceof StdPlayerAI_base))){
					final int x = i;
					final int y = j;
					if(knowNextMove) {
