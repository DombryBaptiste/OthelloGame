...
public class StdOthelloModel implements BoardModel{
	...
	/*public StdOthelloModel(List<Player> p){
		listeners = new EventListenerList();
		started = false;
		game = new HashMap<Point, Player>();
		last = null;
    ...
    public void placeCoin(Point j, Player p) {
    	...
    	fireStateChanged();
    	firePropertyChange(PROP_PLACED, null, true); //optimisation possible ?    	
    	if(isFinish()) {
    		//qui win et update le score du joueur
    		firePropertyChange(PROP_FINISHED, null, true);
    	} else {
    		if (getNextPlayer() instanceof StdPlayerAI_base || getNextPlayer() instanceof StdPlayerAI_normal|| getNextPlayer() instanceof StdPlayer_AI_hard){
